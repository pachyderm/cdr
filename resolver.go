...
func (r *Resolver) deref(ctx context.Context, ref *Ref, cache bool) (io.ReadCloser, error) {
	...
	case *Ref_Http:
		rc, err = r.derefHTTP(ctx, x.Http)
		if err != nil {
			return nil, err
		}
	case *Ref_ContentHash:
		rc, err = r.derefContentHash(ctx, x.ContentHash, false)
		if cache && IsImmutable(ref) {
			rc, err = cacheRC(rc)
		}
	case *Ref_Cipher:
		rc, err = r.derefCipher(ctx, x.Cipher, false)
		if err != nil {
			return nil, err
		}
		if cache && IsImmutable(ref) {
	...
	case *Ref_Compress:
		rc, err = r.derefCompress(ctx, x.Compress, false)
		if err != nil {
			return nil, err
		}
		if cache && IsImmutable(ref) {
	...
	case *Ref_Slice:
		rc, err = r.derefSlice(ctx, x.Slice, cache)
	case *Ref_Concat:
		rc, err = r.derefConcat(ctx, x.Concat, cache)
	default:
